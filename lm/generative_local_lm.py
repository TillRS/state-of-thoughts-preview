...
class Choice(BaseModel, DictAccessMixin):
    ...
    finish_reason: Literal[
        FinishReason.STOP,              # Reached stop phrase
        FinishReason.LENGTH,            # Exceeded max tokens
        FinishReason.CONTENT_FILTER,    # Content filtered (e.g., bad words/guardrail filter)
        FinishReason.TOOL_CALLS         # LM chose to invoke a tool (e.g., calculator/search)
    ] = Field(
        ...,
        description="""
Reason generation stopped: stop, length, content_filter, or tool_calls.
For additional context see:
https://platform.openai.com/docs/api-reference/chat/object#chat-object-choices-finish_reason
""".strip(),
    )
...
class GenerativeLocalVLLM(dspy.BaseLM):
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        self.kwargs = {
            SamplingParam.TEMPERATURE: DEFAULT_TEMPERATURE,
            SamplingParam.N: 1,
        }
    ...
    def _validate_single_inputs(
        ...
        Args:
            ...
            messages: A single conversation thread (list of message dicts, where each dict contains
                ...
                [
                    {MessageKey.ROLE: MessageRole.USER, MessageKey.CONTENT: "What color is the sky?"},
                    {MessageKey.ROLE: MessageRole.ASSISTANT, MessageKey.CONTENT: "The sky is blue."},
                ]
        ...
        if messages is not None:
            ...
            assert all(MessageKey.ROLE in msg and MessageKey.CONTENT in msg for msg in messages), (
    ...
    def forward(
        ...
        **kwargs: Any,
        ...
        if prompt is not None:
            messages = [
                {MessageKey.ROLE: MessageRole.USER, MessageKey.CONTENT: prompt}
            ]
