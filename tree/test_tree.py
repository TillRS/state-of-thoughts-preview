...
from tree.tree_constants import EXISTING_REASONING_STEPS, NodeField, TreeStructure
			...
			state=State(
				...
				reasoning={
					EXISTING_REASONING_STEPS: [
						{QuestionField.REASONING_STEP: f"Step {layer_idx}"},
					],
				},
			),
		)
...
@pytest.mark.parametrize(
	...
	[
		...
		pytest.param(
			{									# init_kwargs
				NodeField.STATE: {QuestionField.QUESTION: "Why is the sky blue?"}
			},
			{},  								# expected_edges_reasoning
		...
		pytest.param(
			{  									# init_kwargs
				NodeField.STATE: {
					ArgumentField.CLAIM: "Tree of thoughts is the best prompting method for LLMs.",
		...
		pytest.param(
			{  									# init_kwargs
				TreeStructure.ROOT: Node(
					index=0,
					layer=0,
					state=State(input={QuestionField.QUESTION: "Why is the sky blue?"}),
				),
			},
			{},  								# expected_edges_reasoning
			[1],  								# expected_nodes_per_layer
			...
			{  									# init_kwargs
				TreeStructure.ROOT: Node(
					...
					state=State(
						...
						reasoning={QuestionField.REASONING_STEP: ["step"]},
					),
				),
			},
			None,  								# expected_edges_reasoning
		...
		pytest.param(
			{  									# init_kwargs
				TreeStructure.ROOT: Node(
					...
					state=State(
						input={QuestionField.QUESTION: "Why is the sky blue?"},
						output={MathField.ANSWER: "Because"},
					),
				),
			},
			None,  								# expected_edges_reasoning
					...
					3: Node(
						...
						state=State(
							...
							reasoning={
								EXISTING_REASONING_STEPS: [
									{QuestionField.REASONING_STEP: "Step 1"},
									{QuestionField.REASONING_STEP: "Step 3"},
								],
							},
						),
					),
				...
				TreeStructure.NODES: {
					...
					1: Node(
						...
						state=State(
							...
							reasoning={
								EXISTING_REASONING_STEPS: [
									{QuestionField.REASONING_STEP: "Step"},
								],
							},
						),
					),
		...
		pytest.param(
			{  									# init_kwargs
				...
				TreeStructure.NODES: "not_a_dict",  			# Invalid type
			},
			None,  								# expected_edges_reasoning
			None,  								# expected_nodes_per_layer
			ValidationError,  					# expected_exception
		...
		pytest.param(
			{  									# init_kwargs
				...
				TreeStructure.EDGES: "not_a_dict",  			# Invalid type
			},
			None,  								# expected_edges_reasoning
			None,  								# expected_nodes_per_layer
			ValidationError,  					# expected_exception
...
@pytest.mark.parametrize(
	...
	[
		...
		pytest.param(
			...
			{QuestionField.REASONING_STEP: "First step"},  	# generator_output
			False,  							# expect_output
			None,  								# expected_exception
		...
		pytest.param(
			...
			{MathField.ANSWER: "Because."},  				# generator_output
			True,  								# expect_output
			None,  								# expected_exception
		...
		pytest.param(
			...
			{MathField.ANSWER: "New"},  					# generator_output
			True,  								# expect_output
			ValidationError,  					# expected_exception
		...
		pytest.param(
			Node(								# parent_node
				...
				state=State(
					...
					reasoning={
						EXISTING_REASONING_STEPS: [
							{QuestionField.REASONING_STEP: "Previous step"},
						],
					},
				),
			),
			...
			{									# generator_output
				QuestionField.REASONING_STEP: "Second step"
			},
			False,  							# expect_output
			None,  								# expected_exception
		...
		pytest.param(
			...
			{MathField.ANSWER: "Final answer"},  			# generator_output
			True,  								# expect_output
			None,  								# expected_exception
...
def test_create_child_node(
	...
	- Stores reasoning steps in EXISTING_REASONING_STEPS when continue_reasoning=True
	...
	if expected_exception:
		...
		with pytest.raises(expected_exception):
			...
			tree.create_child_node(
				...
				reasoning_field_name=QuestionField.REASONING_STEP,
			)
		return
	...
	if expect_output:
		...
		assert EXISTING_REASONING_STEPS in child.state.reasoning
		# EXISTING_REASONING_STEPS contains list of dicts, check if the reasoning step value is in any dict
		reasoning_steps = child.state.reasoning[EXISTING_REASONING_STEPS]
		assert any(
			step.get(QuestionField.REASONING_STEP) == generator_output[QuestionField.REASONING_STEP]
			...
			{  									# nodes
				...
				1: Node(
					...
					state=State(
						...
						reasoning={
							EXISTING_REASONING_STEPS: [
								{QuestionField.REASONING_STEP: "step1"},
							],
						},
					),
				),
				...
				2: Node(
					...
					state=State(
						...
						reasoning={
							EXISTING_REASONING_STEPS: [
								{QuestionField.REASONING_STEP: "step1"},
								{QuestionField.REASONING_STEP: "step2"},
							],
						},
					),
				),
					...
					state=State(
						...
						reasoning={
							EXISTING_REASONING_STEPS: [
								{QuestionField.REASONING_STEP: "step1"},
								{QuestionField.REASONING_STEP: "step2"},
								{QuestionField.REASONING_STEP: "step3"},
							],
						},
					),
				),
					...
					state=State(
						...
						reasoning={
							EXISTING_REASONING_STEPS: [
								...
								{QuestionField.REASONING_STEP: "step2"},
								{QuestionField.REASONING_STEP: "step3"},
								{QuestionField.REASONING_STEP: "step4"},
							],
						},
					),
				),
				...
				3: Node(
					...
					state=State(
						...
						reasoning={
							EXISTING_REASONING_STEPS: [
								{QuestionField.REASONING_STEP: "step1"},
								{QuestionField.REASONING_STEP: "step1a"},
							],
						},
					),
				),
...
@pytest.mark.parametrize(
	...
	[
		pytest.param(
			[										# nodes
				...
				Node(
					...
					state=State(
						input={QuestionField.QUESTION: "Why is the sky blue?"},
						reasoning={QuestionField.REASONING_STEP: ["First step", "Second step"]},
					),
				),
			],
			{										# expected_edges
		...
		pytest.param(
			[									# nodes
				...
				Node(
					...
					state=State(
						...
						reasoning={
							EXISTING_REASONING_STEPS: [
								{"existing_reasoning_steps": "Prior step"},
								{"existing_reasoning_steps": "Latest step"},
							],
						},
					),
				),
			...
			{									# expected_edges
				0: {1: "{'existing_reasoning_steps': 'Latest step'}"},
			},
			{									# expected_children
				0: [1],
			},
		...
		pytest.param(
			[										# nodes
				Node(index=0, layer=0, state=State(input={QuestionField.QUESTION: "Why is the sky blue?"})),
				Node(
					index=1,
					layer=1,
					parent_id=None,
		...
		pytest.param(
			[									# nodes
				Node(index=0, layer=0, state=State(input={QuestionField.QUESTION: "Why is the sky blue?"})),
				Node(index=1, layer=0, state=State(input={QuestionField.QUESTION: "Another?"})),
			],
			None,  								# expected_edges
			None,  								# expected_children
			AssertionError,  					# expected_exception
