...
from signatures.example_signatures import ArgumentField, MathField, TextAnalysisField
...
""".strip()
		...
		pytest.param(
			{							# inputs
				ArgumentField.TOPIC: "Climate change",
				ArgumentField.STANCE: ArgumentStance.PRO.value,
			},
			False,  					# main_request
			(  							# expected_result
				"""
...
def test_format_user_message_content(
	...
	signature = (
		SolveMathProblemWithReasoning
		if MathField.MATH_PROBLEM in inputs
		else GenerateArgumentWithReasoning
	)
...
def test_format_user_message_with_prefix_suffix(
	...
	Args:
	    ...
	    expected_result: The expected formatted user message content.
	"""
	inputs = {str(MathField.MATH_PROBLEM): "What is 5 + 3?"}
	result = adapter.format_user_message_content(
...
@pytest.mark.parametrize(
	...
	[
		pytest.param(
			...
			{									# demo
				ReasoningState.INPUT: {MathField.MATH_PROBLEM: "What is 2 + 2?"},
		...
		pytest.param(
			...
			{									# demo
				ReasoningState.INPUT: {
					TextAnalysisField.INPUT_TEXT: "I love this product! It works perfectly."
				},
				ReasoningState.OUTPUT: {
					TextAnalysisField.SUMMARY: "Customer expresses satisfaction",
					TextAnalysisField.SENTIMENT: "positive",
					TextAnalysisField.KEYWORDS: ["love", "perfectly"],
				},
			},
			(  									# expected_result
			"""
...
def test_format_demos_message_count(
	...
	for i in range(num_demos):
		demos.append(
			{
				ReasoningState.INPUT: {MathField.MATH_PROBLEM: f"What is {i} + {i}?"},
				ReasoningState.OUTPUT: {MathField.ANSWER: str(i + i)},
			}
		)
	...
	for i, msg in enumerate(result):  # Check role alternation
		...
		assert msg[MessageKey.ROLE] == expected_role
...
""".strip()
			...
			{MathField.ANSWER: "4"},		# expected_result
			id="simple_answer",
		),
		pytest.param(
			(								# completion
...
""".strip()
			...
			{								# expected_result
				MathField.ANSWER: "The answer is 42"
			},
			id="verbose_answer",
		),
		pytest.param(
...
""".strip()
			...
			{								# expected_result
				TextAnalysisField.SUMMARY: "Customer is satisfied",
				TextAnalysisField.SENTIMENT: "positive",
				TextAnalysisField.KEYWORDS: ["love", "great"],
			},
...
def test_parse_valid_completions(
	...
	signature = (
		SolveMathProblemWithReasoning
		if MathField.ANSWER in expected_result
...
@pytest.mark.parametrize(
	...
	[
		pytest.param(
			{MathField.ANSWER: "8"},					# json_data
			id="simple_json",
		),
		pytest.param(
			{MathField.ANSWER: "The solution is 42"},	# json_data
			id="verbose_json",
		),
	],
)
def test_parse_json_completions(
	...
	by the adapter. These completions are expected to be in a dictionary format.
	...
	result = adapter.parse(SolveMathProblemWithReasoning, json_completion)
...
def test_parse_fallback_behavior(
	...
	if should_parse_successfully:
		result = adapter.parse(SolveMathProblemWithReasoning, completion)
		assert MathField.ANSWER in result
	else:
		with pytest.raises(AdapterParseError):  # Should raise AdapterParseError
			adapter.parse(SolveMathProblemWithReasoning, completion)
...
@pytest.mark.parametrize(
	...
	[
		pytest.param(
			...
			{									# inputs
				MathField.MATH_PROBLEM: "What is 7 + 8?",
			},
...
""".strip()
		...
		pytest.param(
			[									# demos
				...
				{
					ReasoningState.INPUT: {MathField.MATH_PROBLEM: "3+5"},
					ReasoningState.OUTPUT: {MathField.ANSWER: "8"},
				},
				{
					ReasoningState.INPUT: {MathField.MATH_PROBLEM: "7*8"},
					ReasoningState.OUTPUT: {MathField.ANSWER: "56"},
				},
			],
			{									# inputs
				MathField.MATH_PROBLEM: "What is 7 + 8?",
...
def test_format_message_structure_single_example(
	...
	assert messages[0][MessageKey.ROLE] == MessageRole.SYSTEM
	assert messages[-1][MessageKey.ROLE] == MessageRole.USER
	assert messages[-1][MessageKey.CONTENT] == expected_last_content
...
def test_format_with_response_length_word_constraint(
	...
	Args:
		adapter: The LocalVLLMAdapter instance.
	"""
	inputs = {str(MathField.MATH_PROBLEM): "What is 7 + 8?"}
	response_length = ResponseLength(granularity=GranularityType.WORD, bounds=(5, 10))
	result = adapter.format(
		signature=SolveMathProblemWithReasoning,
		demos=[],
		inputs=inputs,
...
""".strip()
	...
	assert messages[0][MessageKey.CONTENT] == expected_system_content
...
def test_format_with_response_length_sentence_constraint(
	...
	response_length = ResponseLength(
		granularity=GranularityType.SENTENCE, bounds=(1, 3)
	)
	result = adapter.format(
		signature=SolveMathProblemWithReasoning,
		demos=[],
...
""".strip(),
		...
		pytest.param(
			[
				{MathField.MATH_PROBLEM: "What is 5 + 3?"},
				{MathField.MATH_PROBLEM: "What is 10 - 4?"},
			],
			[],
			[
				[
			...
			[
				{
					...
					ReasoningState.OUTPUT: {MathField.ANSWER: "2"},
				}
			],
			[
				[
		...
		pytest.param(
			[
				...
				[
					{
						ReasoningState.INPUT: {
							MathField.MATH_PROBLEM: "What is 3 + 3?"
						},
						ReasoningState.OUTPUT: {MathField.ANSWER: "6"},
					}
				],
			],
			[
	...
	[
		pytest.param(
			...
			[
				{
					ReasoningState.INPUT: {MathField.MATH_PROBLEM: "What is 1 + 1?"},
					ReasoningState.OUTPUT: {"answer": "2"},
				}
			],
			[
				{
					MessageKey.ROLE: "system",
					MessageKey.CONTENT: """
Solve the provided math problem and return its answer.
...
@pytest.mark.parametrize(
	...
	[
		...
		pytest.param(
			{MathField.MATH_PROBLEM: "What is 5 + 3?"},
			{SamplingParam.TEMPERATURE: 0.7, SamplingParam.MAX_TOKENS: 100},
			[],
			[[["## answer\n8"]]],
			{
				"result_length": 1,
		...
		# 1 layer, 1 request, 1 completion
		pytest.param(
			{MathField.MATH_PROBLEM: "What is 12 / 3?"},
			{SamplingParam.TEMPERATURE: 0.0},
			[
				{
					ReasoningState.INPUT: {MathField.MATH_PROBLEM: "What is 6 / 2?"},
					ReasoningState.OUTPUT: {"answer": "3"},
				}
			],
			[[["## answer\n4"]]],
			{
...
@pytest.mark.parametrize(
	...
	[
		...
		pytest.param(
			{MathField.MATH_PROBLEM: "What is 2 * 3?"},
			{SamplingParam.TEMPERATURE: 0.8, SamplingParam.N: 2},
			[[["## answer\n6", "## answer\nSix"]]],
			[[{"answer": "6"}, {"answer": "Six"}]],
...
def test_call_parsing_results(
	...
	assert result == expected_parsed_results
...
@pytest.mark.parametrize(
	...
	[
		pytest.param(
			{MathField.MATH_PROBLEM: "What is 5 + 3?"},
			[{SamplingParam.TEMPERATURE: 0.1}, {SamplingParam.TEMPERATURE: 0.2}],
			ValueError,
...
""".strip(),
		...
		pytest.param(
			...
			[
				{
					ReasoningState.INPUT: {"input_text": "I love this product!"},
					ReasoningState.OUTPUT: {
						TextAnalysisField.SUMMARY: "Customer expresses satisfaction",
						TextAnalysisField.SENTIMENT: "positive",
						TextAnalysisField.KEYWORDS: ["love", "product"],
					},
				}
			],
			[
...
@pytest.mark.parametrize(
	"completion, signature, expected_result",
	[
		pytest.param(
			f"""## {TextAnalysisField.SUMMARY}
Customer is unhappy

## {TextAnalysisField.SENTIMENT}
negative

## {TextAnalysisField.KEYWORDS}
["terrible", "awful"]""",
			AnalyzeTextWithReasoning,
			{
				TextAnalysisField.SUMMARY: "Customer is unhappy",
				TextAnalysisField.SENTIMENT: "negative",
				TextAnalysisField.KEYWORDS: ["terrible", "awful"],
			},
			id="multi_output_parsing",
		),
		pytest.param(
			f'{{"{TextAnalysisField.SUMMARY}": "Brief text", "{TextAnalysisField.SENTIMENT}": "neutral", "{TextAnalysisField.KEYWORDS}": ["text"]}}',
			AnalyzeTextWithReasoning,
			{
				TextAnalysisField.SUMMARY: "Brief text",
				TextAnalysisField.SENTIMENT: "neutral",
				TextAnalysisField.KEYWORDS: ["text"],
			},
			id="json_parsing",
		),
		pytest.param(
			f"## {MathField.ANSWER}\nThe result is 42",
			SolveMathProblemWithReasoning,
			{MathField.ANSWER: "The result is 42"},
...
@pytest.mark.parametrize(
	...
	[
		pytest.param(
			{MathField.MATH_PROBLEM: "What is 5 + 3?"},
			ResponseLength(granularity=GranularityType.WORD, bounds=(5, 10)),
			"Your final answer should be between 5 and 10 words.",
			id="word_constraint",
		),
		pytest.param(
			{MathField.MATH_PROBLEM: "What is 10 / 2?"},
			ResponseLength(granularity=GranularityType.SENTENCE, bounds=(1, 2)),
			"Your final answer should be between 1 and 2 sentences.",
			id="sentence_constraint",
		),
		pytest.param(
			{MathField.MATH_PROBLEM: "What is 7 * 6?"},
			None,
