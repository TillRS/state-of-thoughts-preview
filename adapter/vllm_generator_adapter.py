...
from misc_utils import is_list_of_lists, parse_base_signature, parse_reasoning_signature
...
class BatchFormatterError(Exception):
	"""Custom exception for batch formatting errors."""
...
class VLLMGeneratorAdapter(LocalVLLMAdapter):
	...
	def __init__(
		...
		thinking_start_tag: str = XMLTag.THINKING_START,
		thinking_end_tag: str = XMLTag.THINKING_END,
		reasoning_step_start_tag: str = XMLTag.STEP_START,
		reasoning_step_end_tag: str = XMLTag.STEP_END,
		answer_start_tag: str = XMLTag.ANSWER_START,
		answer_end_tag: str = XMLTag.ANSWER_END,
	...
	def __call__(
		...
		Args:
			...
			inputs: The inputs to use for generation and reasoning trajectories (that each include these
				input fields along with partial reasoning towards the answer).
			...
			lm_kwargs: The keyword arguments to pass to the language model. For example:
				...
				- A list of dicts (on per trajectory): [{"temperature": 0.7, "n": 5}, {"temperature": 0.7, "n": 3}, ...]
				- A list of lists of dicts (outer list per trajectory, inner list per intervention):
					[[{"temperature": 0.7, "n": 5}, {"temperature": 0.7, "n": 3}], [{"temperature": 0.8, "n": 2}], ...]
					This structure matches prefix_for_output, continue_reasoning, and internal_reasoning_for_output.
			...
			previous_content: List of previous reasoning content strings, one per state. None if no previous content.
			internal_reasoning_for_output: List of lists of internal reasoning strings. Outer list is per state,
				inner list is per intervention. None if no internal reasoning.
			prefix_for_output: List of lists of prefix strings. Outer list is per state,
		...
		final_trajectory_message_role = new_reasoning_trajectories[0][-1][MessageKey.ROLE]
		assert all(
			trajectory[-1][MessageKey.ROLE] == final_trajectory_message_role
			for trajectory in new_reasoning_trajectories
		), (
			f"All trajectories should have final messages with the same role.\n"
			f"Got the following roles for final messages: {[trajectory[-1][MessageKey.ROLE] for trajectory in new_reasoning_trajectories]}"
		)
		continue_final_message = final_trajectory_message_role == MessageRole.ASSISTANT
		...
		if is_list_of_lists(lm_kwargs, dict):
			assert len(lm_kwargs) == len(continue_reasoning) and all(
				...
				f"If lm_kwargs is list[list[dict]], it must contain exactly one dict per intervention per trajectory, got {type(lm_kwargs)}:\n{lm_kwargs}"
			)
			lm_kwargs_list = []
			for trajectory_kwargs in lm_kwargs:
				lm_kwargs_list.extend(trajectory_kwargs)

		elif isinstance(lm_kwargs, list):
			assert len(lm_kwargs) == len(continue_reasoning), (
				f"If lm_kwargs is list[dict], it must contain exactly one dict per trajectory, got {type(lm_kwargs)}:\n{lm_kwargs}"
			)
			lm_kwargs_list = []
			for i in range(len(lm_kwargs)):
				lm_kwargs_list.extend([lm_kwargs[i]] * len(continue_reasoning[i]))

		else:
			assert isinstance(lm_kwargs, dict), (
				f"If lm_kwargs is neither list[dict] nor list[list[dict]], it must be exactly one dict, got {type(lm_kwargs)}:\n{lm_kwargs}"
			)
			lm_kwargs_list = [lm_kwargs] * num_messages
		...
		# Build sampling parameters for each message
		sampling_params_list = []
		remaining_kwargs_list = []

		for i, kwargs in enumerate(lm_kwargs_list):
			kwargs_copy = kwargs.copy()

			# Determine stop tokens for this specific message
			# Use the trajectory mapping populated in format()
			current_reasoning = self._trajectory_continue_reasoning[i]
			stop_tokens = self._determine_stop_tokens(current_reasoning)

			# Extract and prepare sampling parameters
			sampling_params, remaining_kwargs = self._get_sampling_params(kwargs_copy)
			current_params = self._extract_sampling_params_as_dict(sampling_params)
			current_params[SamplingParam.STOP] = stop_tokens
		...
		for i, trajectory in enumerate(new_reasoning_trajectories, 1):
			...
			for msg in trajectory:
				logger.debug(f"\n[{msg[MessageKey.ROLE].upper()}]:\n{ msg[MessageKey.CONTENT]}\n")
		logger.debug(f"{'='*80}\n")
		...
		if len(new_reasoning_trajectories) == 1:
			response = lm.forward(
				messages=new_reasoning_trajectories[0],
				sampling_params=sampling_params_list[0] if sampling_params_list else None,
		...
		# Extract text from ChatCompletionResponse objects
		# Format: list[list[str | dict]] = batch[completions[text or {text, logprobs}]]
		outputs: list[list[str | dict[str, Any]]] = []
		for resp in responses:
			completions: list[str | dict[str, Any]] = []
			for choice in resp.choices:
				if hasattr(choice, "logprobs") and choice.logprobs is not None:
					completions.append({"text": choice.text, "logprobs": choice.logprobs})
				else:
	...
	def _call_post_process(
		...
		for example_idx, example in enumerate(outputs):
			...
			for candidate in example:
				...
				full_text = prefill + candidate_text if prefill else candidate_text
				...
				try:
					...
					value = {
						AdapterErrorKey.FAILED_PARSING: True,
						AdapterErrorKey.RAW_OUTPUT: full_text,
						AdapterErrorKey.ERROR: str(e),
					}
		...
		for k, v in processed_sections:
			...
			if k is not None and (k in target_fields.keys()):
				try:
					...
					fields[k] = parse_value(v.strip(), annotation)
	...
	def _extract_sampling_params_as_dict(
		self, sampling_params: SamplingParams
	) -> dict[str, Any]:
		"""
		Extract SamplingParams attributes as a dictionary.

		Args:
		    sampling_params: SamplingParams instance

		Returns:
		    Dictionary of sampling parameter names to values
		"""
		params_dict = {}
		for param_name in set(SamplingParams.__annotations__.keys()):
			if hasattr(sampling_params, param_name):
				params_dict[param_name] = getattr(sampling_params, param_name)
		return params_dict
	...
	def format(
		...
		for trajectory_index, trajectory_continue_reasoning_list in enumerate(
			...
			has_internal_reasoning = (
				trajectory_previous_content
				and ControllerOutputParameters.INTERNAL_REASONING in trajectory_previous_content
			...
			for intervention_idx, intervention_messages in enumerate(
				...
				if intervention_messages[-1][MessageKey.ROLE] == MessageRole.ASSISTANT:
					self._current_assistant_prefills.append(
						intervention_messages[-1][MessageKey.CONTENT]
					)
				else:
					self._current_assistant_prefills.append("")
		...
		base_messages.append(
			{
				...
				MessageKey.CONTENT: self.create_system_prompt(
					signature=signature,
					thought_length=thought_length,
					response_length=response_length,
					has_internal_reasoning=has_internal_reasoning,
		...
		for i in range(num_interventions):
			...
			if assistant_content:
				messages.append({
					...
					MessageKey.CONTENT: assistant_content,
				})
	...
	def format_continued_assistant_message(
		...
		if not stripped_content and continue_reasoning is False:
			...
			parts = [stripped_content] if stripped_content else [str(XMLTag.THINKING_START)]
		if continue_reasoning:
			# If previous content ends with </thinking>, remove it to add more steps
			while parts[0].strip().endswith(XMLTag.THINKING_END):
				parts[0] = parts[0].rstrip().removesuffix(str(XMLTag.THINKING_END)).rstrip()
			# If previous content contains an incomplete step, close it first
			# (check after removing </thinking> tag)
			if (
				parts[0].strip()
				and XMLTag.STEP_START in parts[0]
				and not parts[0].strip().endswith(XMLTag.STEP_END)
			):
				parts.append(str(XMLTag.STEP_END))
			parts.append(str(XMLTag.STEP_START))
			...
			if parts:
				...
				if (
					...
					and XMLTag.STEP_START in parts[0]
					and not parts[0].strip().endswith(XMLTag.STEP_END)
					and not parts[0].strip().endswith(XMLTag.THINKING_END)
				):
					parts.append(str(XMLTag.STEP_END))
				# Close thinking tag if needed
				if not parts[0].strip().endswith(XMLTag.THINKING_END):
					parts.append(str(XMLTag.THINKING_END))
			# Add answer section (regardless of whether we had previous content)
			parts.append(str(XMLTag.ANSWER_START))
	...
	def user_message_output_requirements(
		...
		if has_internal_reasoning:
			step_instruction = f"Each `{XMLTag.STEP_START}` section should include a `## {ControllerOutputParameters.INTERNAL_REASONING}` section (guidance provided to help your thinking), followed by a `## {reasoning_field_name}` section."
		else:
			step_instruction = f"Each `{XMLTag.STEP_START}` section should contain a `## {reasoning_field_name}` section."

		# Format instructions for answer section
		answer_instruction = f"The `{XMLTag.ANSWER_START}` section should include sections for {output_fields}."
		...
		format_guidance = (
			...
			f"1. Begin with `{XMLTag.THINKING_START}...{XMLTag.THINKING_END}` tags. Inside these tags, include multiple `{XMLTag.STEP_START}...{XMLTag.STEP_END}` sections for your {reasoning_field_name}s.\n"
			f"\t{step_instruction}\n"
			f"2. After the `{XMLTag.THINKING_END}` tag, include your final answer within `{XMLTag.ANSWER_START}...{XMLTag.ANSWER_END}` tags.\n"
	...
	def format_demo_assistant_message(
		...
		# Build message parts
		message_parts = []
		message_parts.append(XMLTag.THINKING_START)
		# Add reasoning steps
		reasoning_steps = demo[ReasoningState.REASONING]
		for step_dict in reasoning_steps:
			message_parts.append(XMLTag.STEP_START)
			# Include internal_reasoning only if enabled and present in demo (controller guidance)
			if has_internal_reasoning:
				assert ControllerOutputParameters.INTERNAL_REASONING in step_dict, (
					f"Expected internal reasoning field '{ControllerOutputParameters.INTERNAL_REASONING}' in demo reasoning step, but it is missing. "
					"Ensure that has_internal_reasoning is set correctly based on the demo data."
				)
				message_parts.append(
					f"## {ControllerOutputParameters.INTERNAL_REASONING}\n{step_dict[ControllerOutputParameters.INTERNAL_REASONING]}"
				)
			...
			message_parts.append(XMLTag.STEP_END)

		message_parts.append(XMLTag.THINKING_END)
		message_parts.append(XMLTag.ANSWER_START)
		...
		message_parts.append(XMLTag.ANSWER_END)
		return "\n".join(message_parts)
