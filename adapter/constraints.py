...
# Local imports
from signatures.field_constants import DEFAULT_REASONING_FIELD_NAME, ConstraintField


class GranularityType(StrEnum):
	"""
	A representation of granularity for text-based responses.

	Each granularity type corresponds to a specific unit of measurement for text length:
		- `word`: A single word.
		- `sentence`: A complete sentence.
		- `paragraph`: A complete paragraph.

	This enum is often used to define constraints for how long text-based responses
	should be (either within a certain range, or an exact number) based on the granularity type.
	"""
...
class ResponseLength(BaseModel):
	...
	granularity: GranularityType = Field(
		...
		default=GranularityType.SENTENCE,
	)
...
""".strip(),
	...
	@field_validator(ConstraintField.BOUNDS)
	@classmethod
	...
	def __str__(self) -> str:
		...
		granularity_val = self.granularity.value
...
def format_thought_length_instruction(
	...
	if use_internal_reasoning_for_thought_generation and thought_length is not None:
		granularity_value = thought_length.granularity.value
