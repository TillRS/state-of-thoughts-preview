...
def test_get_reasoning_fields() -> None:
	...
	assert MathField.MATH_OPERATION in reasoning_fields

	# Just verify that we can identify this as a reasoning field
	assert (
		MathWithReasoningSignature.reasoning_fields[MathField.MATH_OPERATION]
		is not None
	)
...
def test_reasoning_signature_structure() -> None:
	...
	assert list(MathWithReasoningSignature.input_fields.keys()) == [
		MathField.MATH_PROBLEM
	]
	assert list(MathWithReasoningSignature.reasoning_fields.keys()) == [
		MathField.MATH_OPERATION
	]
	assert list(MathWithReasoningSignature.output_fields.keys()) == [MathField.ANSWER]
...
@pytest.mark.parametrize(
	...
	[
		pytest.param(
			FinalOutputKind.SYNTHESIS_STRICT,
			FINAL_OUTPUT_SYNTHESIS_STRICT,
			id="synthesis_strict",
		),
		pytest.param(
		...
		pytest.param(
			FinalOutputKind.SYNTHESIS_RESTRUCTURED,
			FINAL_OUTPUT_SYNTHESIS_RESTRUCTURED,
...
def test_get_final_output_description_valid_kinds(
	final_output_kind: FinalOutputKind, expected_description: str
) -> None:
	"""
	Test get_final_output_description returns correct prompt for each FinalOutputKind.

	This test verifies that the function correctly maps each FinalOutputKind enum value
	to its corresponding prompt string from adapter.prompts.

	Parameters:
	    final_output_kind: The FinalOutputKind enum value to test.
...
def test_get_final_output_description_invalid_kind() -> None:
	"""
	Test get_final_output_description raises ValueError for invalid FinalOutputKind.
	...
	We use an integer that doesn't correspond to any valid FinalOutputKind enum value.
	"""
