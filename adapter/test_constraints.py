...
def test_fails_if_lower_bound_is_zero_or_negative() -> None:
	...
	with pytest.raises(ValueError):
		ResponseLength(granularity=GranularityType.WORD, bounds=(0, 10))

	with pytest.raises(ValueError):
		ResponseLength(granularity=GranularityType.SENTENCE, bounds=(-5, 10))
...
def test_fails_if_upper_bound_is_less_than_lower_bound() -> None:
	...
	"""
	with pytest.raises(ValueError):
		ResponseLength(granularity=GranularityType.WORD, bounds=(10, 5))
...
@pytest.mark.parametrize(
	...
	[
		pytest.param(
			ResponseLength(granularity=GranularityType.WORD, bounds=(10, 20)),
			"Your final answer should be between 10 and 20 words.",
			id="between_10_and_20_words",
		),
		pytest.param(
			ResponseLength(granularity=GranularityType.SENTENCE, bounds=(1, 5)),
			"Your final answer should be between 1 and 5 sentences.",
			id="between_1_and_5_sentences",
		),
		pytest.param(
			ResponseLength(granularity=GranularityType.PARAGRAPH, bounds=(3, 5)),
			"Your final answer should be between 3 and 5 paragraphs.",
			id="between_3_and_5_paragraphs",
		),
		pytest.param(
			ResponseLength(granularity=GranularityType.WORD, bounds=(5, 5)),
			"Your final answer should be exactly 5 words.",
			id="exactly_5_words",
		),
		pytest.param(
			ResponseLength(granularity=GranularityType.SENTENCE, bounds=(1, 1)),
			"Your final answer should be exactly 1 sentence.",
			id="exactly_1_sentence",
		),
		pytest.param(
			ResponseLength(granularity=GranularityType.WORD, bounds=(1, 10)),
