...
# Local imports
from signatures.example_signatures import MathField, QuestionField
...
class TestBasicSignatureCreation:
	...
	def test_signature_with_reasoning_fields(self) -> None:
		...
		assert field_names == [QuestionField.QUESTION, "reasoning", MathField.ANSWER]
...
class TestStringSignatureParsing:
	...
	def test_single_arrow_signature_parsing(self) -> None:
		...
		assert QuestionField.QUESTION in sig.input_fields
		assert "context" in sig.input_fields
		assert MathField.ANSWER in sig.output_fields
	...
	def test_typed_signature_parsing(self) -> None:
		...
		# Verify field types
		question_field = sig.input_fields[QuestionField.QUESTION]
		count_field = sig.input_fields["count"]
		answer_field = sig.output_fields[MathField.ANSWER]
