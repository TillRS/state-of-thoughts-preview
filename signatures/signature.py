...
from signatures.field_constants import FieldType
	...
	def _validate_fields(cls):
		...
		for name, field in model_fields.items():
			if hasattr(field, "json_schema_extra") and field.json_schema_extra:
				...
				if field_type in [FieldType.INPUT, FieldType.REASONING, FieldType.OUTPUT]:
...
class ReasoningSignature(BaseModel, metaclass=ReasoningSignatureMeta):
	...
	def with_updated_fields(
		...
		existing_extra = fields_copy[name].json_schema_extra or {}
		if isinstance(existing_extra, dict):
			...
			fields_copy[name].json_schema_extra = kwargs
