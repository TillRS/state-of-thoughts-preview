...
# Local imports
from signatures.field_constants import FieldMetadata, FieldType
...
def move_kwargs(**kwargs):
	...
	for k, v in kwargs.items():
		if k in DSPY_FIELD_ARG_NAMES:
			...
			pydantic_kwargs[k] = v
	if (
		FieldMetadata.DESCRIPTION in kwargs
		and FieldMetadata.DESC not in json_schema_extra
	):
		json_schema_extra[FieldMetadata.DESC] = kwargs[FieldMetadata.DESCRIPTION]
	constraints = _translate_pydantic_field_constraints(**kwargs)
	if constraints:
		json_schema_extra[FieldMetadata.CONSTRAINTS] = constraints
	pydantic_kwargs[FieldMetadata.JSON_SCHEMA_EXTRA] = json_schema_extra
...
def InputField(**kwargs):  # noqa: N802
	...
	"""
	return pydantic.Field(**move_kwargs(**kwargs, __dspy_field_type=FieldType.INPUT))
...
def ReasoningField(**kwargs):  # noqa: N802
	...
	return pydantic.Field(
		**move_kwargs(**kwargs, __dspy_field_type=FieldType.REASONING)
	)
...
def OutputField(**kwargs):  # noqa: N802
	...
	"""
	return pydantic.Field(**move_kwargs(**kwargs, __dspy_field_type=FieldType.OUTPUT))


def new_to_old_field(field):
	"""Convert new pydantic Field to old field format."""
	field_type = field.json_schema_extra[FieldMetadata.DSPY_TYPE]

	if field_type == FieldType.INPUT:
		return OldInputField(
			prefix=field.json_schema_extra.get(FieldMetadata.PREFIX),
			desc=field.json_schema_extra.get(FieldMetadata.DESC),
			format=field.json_schema_extra.get(FieldMetadata.FORMAT),
		)
	elif field_type == FieldType.REASONING:
...
class OldField:
	...
	def __repr__(self):
		return f"{self.__class__.__name__}({FieldMetadata.PREFIX}={self.prefix}, {FieldMetadata.DESC}={self.desc})"
