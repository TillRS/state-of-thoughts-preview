...
## Example Signatures (`example_signatures.py`)

Pre-built signatures for common tasks:
...
## Usage with Tree of Thoughts

Signatures integrate with the Tree of Thoughts framework:
...
# Initialize with a ReasoningSignature
tot = TreeOfThoughts(
...
The Generator uses the signature to:
1. Format prompts with the input fields
2. Parse reasoning steps into the `ReasoningField`
3. Generate final output into the `OutputField`
...
signatures/
├── __init__.py
├── signature.py           # ReasoningSignature class
├── field.py               # ReasoningField, rubric_weight
├── field_constants.py     # Field type constants
├── example_signatures.py  # Pre-built signatures
├── test_field.py          # Field unit tests
├── test_signature.py      # Signature unit tests
└── README.md              # This file
```
