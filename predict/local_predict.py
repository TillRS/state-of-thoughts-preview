...
from lm.lm_constants import DEFAULT_TEMPERATURE
from misc_utils import _is_instance_of_type, _is_optional_type
...
class LocalPredict(dspy.Predict):
	...
	def __init__(
		...
		**config,
		...
		Args:
		    ...
		    **config: Hyperparameters for the language model such as `n` and `temperature`, might be overridden at runtime.
		"""
	...
	def _forward_preprocess(self, **kwargs) -> ForwardInputs:
		...
		# Always extract config/lm from first batch input (they should be identical across batch)
		...
		# Handle temperature and multiple generations
		...
		# Only apply fallback logic if temperature was not explicitly set in config
		# This allows explicit temperature=0.0 for deterministic behavior
		if "temperature" not in config:
			# Temperature not explicitly set in config, check LM default
			if temperature is None or temperature <= 0.15:
				self.logger.warning(
					f"No temperature specified and LM default ({temperature}) too low. Setting to {DEFAULT_TEMPERATURE}."
				)
				config["temperature"] = DEFAULT_TEMPERATURE
			else:
				config["temperature"] = temperature
		else:
			# Temperature explicitly set in config - always respect it
			config["temperature"] = temperature
	...
	def forward(self, **kwargs) -> list[Prediction]:
		...
		adapter = settings.adapter or LocalVLLMAdapter()
