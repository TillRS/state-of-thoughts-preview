...
class ControllerPrediction(dspy.Prediction):
	...
	def __init__(
		...
		Parameters:
			...
			considerations: Justification for why this tool and arguments were chosen.
			tool_execution_error: Error message if tool execution failed. Defaults to "".
...
def create_reasoning_intervention_from_choices(
	...
	for config in configs:
		...
		if ControllerOutputParameters.INTERNAL_REASONING in choice_spec:
			internal_reasoning_parts.append(
				choice_spec[ControllerOutputParameters.INTERNAL_REASONING]
			)

		# Collect prefix if present
		if ControllerOutputParameters.PREFIX in choice_spec:
			prefix_parts.append(choice_spec[ControllerOutputParameters.PREFIX])
...
def load_action_space_json(json_path: str | Path) -> ActionSpaceConfig:
	...
	if ActionSpaceJsonKeys.DIMENSION_NAME not in data:
		raise ValueError(
			f"Action space JSON missing '{ActionSpaceJsonKeys.DIMENSION_NAME}' field: {path}"
		)
	if ActionSpaceJsonKeys.DIMENSION_DEFINITION not in data:
		raise ValueError(
			f"Action space JSON missing '{ActionSpaceJsonKeys.DIMENSION_DEFINITION}' field: {path}"
		)
	if ActionSpaceJsonKeys.DIMENSION_CHOICES not in data:
		raise ValueError(
			f"Action space JSON missing '{ActionSpaceJsonKeys.DIMENSION_CHOICES}' field: {path}"
		)

	return ActionSpaceConfig(
		name=sanitize_param_name(data[ActionSpaceJsonKeys.DIMENSION_NAME]),
		definition=data[ActionSpaceJsonKeys.DIMENSION_DEFINITION],
		choices=data[ActionSpaceJsonKeys.DIMENSION_CHOICES],
	)
...
def execute_tool_safely(
	...
	try:
		...
		if isinstance(result, ReasoningIntervention):
			...
			intervention = ReasoningIntervention(
				continue_reasoning=result[ControllerContinueReasoningChoice.CONTINUE_REASONING],
				internal_reasoning=result[ControllerOutputParameters.INTERNAL_REASONING],
				prefix=result[ControllerOutputParameters.PREFIX],
			)
		return intervention, ""
...
def remove_duplicate_actions_with_counts(
	...
	"""
	Remove duplicates from a list of dictionaries based on ControllerActionParameters.ACTION and ControllerActionParameters.ARGUMENTS,
	and annotate each unique dict with ControllerOutputParameters.UNIQUE_ACTION_RESPONSE_COUNT.
	...
	for d in output_dictionaries:
		# Create a key by converting ControllerActionParameters.ARGUMENTS to a sorted string representation
		action_key = d[ControllerActionParameters.ACTION]
		args_key = (
			str(sorted(d[ControllerActionParameters.ARGUMENTS].items()))
			if d[ControllerActionParameters.ARGUMENTS]
			else ""
		)
		key = (action_key, args_key)

		if key in seen:
			seen[key][ControllerOutputParameters.UNIQUE_ACTION_RESPONSE_COUNT] += 1
		else:
			seen[key] = {**d, ControllerOutputParameters.UNIQUE_ACTION_RESPONSE_COUNT: 1}
