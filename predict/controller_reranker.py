...
class TreeOfThoughtsControllerReranker(dspy.Module):
	...
	def create_tool(configs: list[ActionSpaceConfig], choices: dict[str, str]) -> dspy.Tool:
		...
		for config in configs:
			...
			choice_def: str = choice_spec[ControllerOutputParameters.DEFINITION]
	...
	def _score_actions(
		...
		input_dict = {**state.model_dump()[ReasoningState.INPUT]}
		existing_reasoning = state.reasoning.get(self.reasoning_field_name, [])

		# Convert reasoning to Reasoning format for adapter
		reasoning_candidates = [{self.reasoning_field_name: existing_reasoning or []}]
		...
		rerank_responses: list[RerankResponse] = self.scoring_adapter(
			...
			lm=reranker_lm,
			input=input_dict,
			scoring_target=ScoringTarget.ACTION,
			...
			output_dictionaries.append(
				{
					...
					ControllerOutputParameters.TOOL_DESCRIPTIONS: tool_descriptions,
					ControllerActionParameters.CONSIDERATIONS: considerations,
					ControllerOutputParameters.SCORE: score,
				}
			)
	...
	def forward(
		...
		**kwargs,
		...
		Parameters:
		    ...
		    demos (list[dict[str, Any]] | None): Optional demos for few-shot examples.
		    **kwargs: Additional keyword arguments (e.g., 'lm', 'temperature',
		        'max_tokens', 'controller_sampling_params'). Note that sampling
		        parameters (temperature, max_tokens, controller_sampling_params) are ignored
		        for reranking. If the language model is not provided as a keyword argument,
		        it must be set via set_lm() before calling forward().
		...
		if "generative_lm" in kwargs:
			...
			logger.warning(
				"generative_lm parameter is deprecated for reranker controller. "
				"The reranker now scores complete action-argument combinations."
			)
		...
		for i, state in enumerate(states):
			forced_result_list = self.forced_choice_function(self.tools, state)
				...
				forced_preds = [
					{
						...
						ControllerOutputParameters.TOOL_DESCRIPTIONS: self.tools[action_name].desc,
						# Assign the highest score to forced choices
						ControllerOutputParameters.SCORE: 1.0,
					}
		...
		if reranker_states_with_indices:
			for original_idx, state in reranker_states_with_indices:
				...
				output_dictionaries = remove_duplicate_actions_with_counts(
					output_dictionaries
				)
		...
		for output_dicts in final_controller_outputs:
			...
			for output_dict in output_dicts:
				tool_name = output_dict[ControllerActionParameters.ACTION]
				tool = self.tools[tool_name]
				intervention, tool_execution_error = execute_tool_safely(
					tool, output_dict[ControllerActionParameters.ARGUMENTS]
				)
				state_predictions.append(
					ControllerPrediction(
						tool=tool,
						chosen_values=output_dict[ControllerActionParameters.ARGUMENTS],
						intervention=intervention,
						considerations=output_dict[ControllerActionParameters.CONSIDERATIONS],
						tool_execution_error=tool_execution_error,
						num_occurrences=output_dict.get(
							ControllerOutputParameters.UNIQUE_ACTION_RESPONSE_COUNT, 1
						),
