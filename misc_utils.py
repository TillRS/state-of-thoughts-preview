...
USE_VLLM_STUBS = os.environ.get("DSPY_USE_VLLM_STUBS") == "1"
...
def create_model_config(**kwargs: dict[str, Any] | None) -> dict[str, Any]:
	"""
	Optimize model configuration based on constants.GPU type and model size.
	...
	Potential kwargs:
	    ...
	    * gpu: The constants.GPU type to use for the model (e.g., "A100_40GB", "A100_80GB", "T4", etc.).
	    ...
	    * gpu_memory_utilization: A float between 0 (low) and 1 (high) denoting how much constants.GPU memory
	    ...
	    * swap_space: The size (GiB) of CPU memory per constants.GPU to use as swap space.
	    * cpu_offload_gb: The size (GiB) of CPU memory to use for offloading the model weights.
	    * verbose: Whether to print verbose output during model loading and inference.
	...
	if dtype:
		...
		# We let vLLM choose the best dtype based on the model name (mapping to whether it was
		# trained with fp8 or bf16).
		config["dtype"] = "auto"
